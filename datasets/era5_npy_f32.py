...
Years = {
    'train': ['2012-02-13 00:00:00', '2016-12-31 23:00:00'],
    'valid': ['2016-01-01 00:00:00', '2016-12-31 23:00:00'],
    'test': ['2017-01-01 00:00:00', '2017-12-31 23:00:00'],
...
multi_acc_mean_dir = "./data/era5/climate_mean_day/1993-2016"
single_acc_mean_dir = "./data/era5/climate_mean_day/single/1993-2016"


class era5_npy_f32(Dataset):
    def __init__(self, data_dir='./data/era5', split='train', **kwargs) -> None:
        super().__init__()
        ...
        self.obs_type = kwargs.get("obs_type", 'simulation')
        self.gdas_obs_dir = kwargs.get("gdas_obs_dir", None)
        ...
        self.dim = len(self.single_level_vnames) + len(self.multi_level_vnames) * len(self.height_level_list)
        ...
        if self.rm_equator:
            self.a = np.zeros((self.dim, 720, 1440), dtype=np.float32)
        else:
            self.a = np.zeros((self.dim, 721, 1440), dtype=np.float32)
    ...
    def __len__(self):
        ...
        if self.split != "test":
            ...
            data_len -= self.pred_length * self.sample_stride
    ...
    def __getitem__(self, index):
        ...
        tar_idx = np.array([index + i * self.sample_stride for i in range(self.length)])
        # print(f'__getitem__ {self.file_list[index + (self.length - 1) * self.sample_stride]}')
        file = self.file_list[index + (self.length-1) * self.sample_stride]
        
        if self.obs_type == 'gdas':
            if os.path.exists(f"{self.gdas_obs_dir}/{file}-obs.npy"):
                obs = np.load(f"{self.gdas_obs_dir}/{file}-obs.npy")
                mask = np.load(f"{self.gdas_obs_dir}/{file}-mask.npy")
            else:
                obs = np.zeros_like(self.a, dtype=self.a.dtype)
                mask = np.zeros_like(self.a, dtype=self.a.dtype)
            array_seq.append(obs)
            array_seq.append(mask)

        return array_seq, tar_idx, file
    
    def getitem(self, index):
        index = min(index, len(self.file_list) - (self.length-1) * self.sample_stride - 1)
        array_seq = self.get_data([index + (self.length - 1) * self.sample_stride])
        tar_idx = np.array([index + self.sample_stride * (self.length - 1)])
        # print(f'getitem {tar_idx}')
